...
func TestForEachTaskMessageFactory_CreateMessage(t *testing.T) {
	type args struct {
		trc ylem_messaging.TaskRunContext
	}

	var mf *ylem_messaging.ForEachTaskMessageFactory
	...
	correctTaskImplementation := &task.ForEach{Uuid: "20469af1-3c50-499f-8b76-7d2ae0c66575"}
	...
	correctMessageEnvelope := &messaging.Envelope{
		Headers: map[string]string{"X-Message-Name": "tasks.run_for_each"},
	...
	tests := []struct {
		name    string
		f       *ylem_messaging.ForEachTaskMessageFactory
		args    args
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err != nil) != tt.wantErr {
				t.Errorf("ForEachTaskMessageFactory.CreateMessage() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("ForEachTaskMessageFactory.CreateMessage() = %v, want %v", got, tt.want)
			}
		})
	}
}
