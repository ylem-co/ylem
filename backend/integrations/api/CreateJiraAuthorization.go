...
func CreateJiraAuthorization(w http.ResponseWriter, r *http.Request) {
	...
	auth := entities.JiraAuthorization{
		Name:             "New Jira Authorization",
	...
	err = repositories.CreateJiraAuthorization(db, &auth)
	if err != nil {
		log.Error(err.Error())
		helpers.HttpReturnErrorInternal(w)
	...
	createAndWriteJiraGrantLink(w, auth)
}
