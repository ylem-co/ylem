...
func TestProcessorTaskMessageFactory_CreateMessage(t *testing.T) {
	type args struct {
		trc ylem_messaging.TaskRunContext
	}

	var mf *ylem_messaging.ProcessorTaskMessageFactory
	...
	correctTaskImplementation := &task.Processor{Uuid: "20469af1-3c50-499f-8b76-7d2ae0c66575", Expression: "id > 0"}
	...
	correctMessageEnvelope := &messaging.Envelope{
		Headers: map[string]string{"X-Message-Name": "tasks.process_data"},
	...
	tests := []struct {
		name    string
		f       *ylem_messaging.ProcessorTaskMessageFactory
		args    args
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err != nil) != tt.wantErr {
				t.Errorf("ProcessorTaskMessageFactory.CreateMessage() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("ProcessorTaskMessageFactory.CreateMessage() = %v, want %v", got, tt.want)
			}
		})
	}
}
