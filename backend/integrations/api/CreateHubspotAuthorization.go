...
func CreateHubspotAuthorization(w http.ResponseWriter, r *http.Request) {
	log.Tracef("Creating Hubspot Authorization")
	...
	auth := entities.HubspotAuthorization{
		Name:             "New Hubspot Authorization",
	...
	err = repositories.CreateHubspotAuthorization(db, &auth)
	if err != nil {
		log.Error(err.Error())
		helpers.HttpReturnErrorInternal(w)
	...
	createAndWriteHubspotGrantLink(w, auth)
}
