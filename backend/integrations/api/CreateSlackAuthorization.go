...
func CreateSlackAuthorization(w http.ResponseWriter, r *http.Request) {
	log.Tracef("Creating Slack Authorization")
	...
	auth := entities.SlackAuthorization{
		Name:             "New Slack Authorization",
	...
	err = repositories.CreateSlackAuthorization(db, &auth)
	if err != nil {
		log.Error(err.Error())
		helpers.HttpReturnErrorInternal(w)
	...
	createAndWriteSlackGrantLink(w, auth)
}
