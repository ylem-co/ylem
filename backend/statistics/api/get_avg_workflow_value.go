...
func PipelineAvgValueFunc(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	...
	value, err := rm.GetPipelineRunResultAvg(uuidParam, readmodel.Period(periodParamStr), perdiodCountParam)
	if err != nil {
		log.Error(err)
		helpers.HttpReturnErrorInternal(w)
		return
