...
func TestExternalTriggerTaskMessageFactory_CreateMessage(t *testing.T) {
	type args struct {
		trc ylem_messaging.TaskRunContext
	}

	var mf *ylem_messaging.ExternalTriggerMessageFactory
	...
	correctTaskImplementation := &task.ExternalTrigger{Uuid: "20469af1-3c50-499f-8b76-7d2ae0c66575"}
	correctTask := &task.Task{
		...
		Type: task.TaskTypeExternalTrigger, 
	...
	correctMessageEnvelope := &messaging.Envelope{
		Headers: map[string]string{"X-Message-Name": "tasks.external_trigger"},
	...
	tests := []struct {
		name    string
		f       *ylem_messaging.ExternalTriggerMessageFactory
		args    args
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err != nil) != tt.wantErr {
				t.Errorf("ExternalTriggerMessageFactory.CreateMessage() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("ExternalTriggerMessageFactory.CreateMessage() = %v, want %v", got, tt.want)
			}
		})
	}
}
