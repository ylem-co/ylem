...
func ShowSQLIntegrationDatabases(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		log.Errorf("could not collect databases: couldn't create integration connection: %s", err.Error())
	...
	if _, ok := conn.(sql.DescribableConnection); !ok {
		helpers.HttpReturnErrorBadRequest(w, "the integration doesn't support databases describe", &[]string{})
	...
	if err != nil {
		log.Errorf("could not collect databases: couldn't open integration connection: %s", err.Error())
	...
	if err != nil {
		log.Errorf("could not collect databases: %s", err.Error())
	...
	if err != nil {
		log.Errorf("could not marshal databases list: %s", err.Error())
		helpers.HttpReturnErrorInternal(w)
