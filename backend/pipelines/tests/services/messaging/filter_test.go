...
func TestFilterTaskMessageFactory_CreateMessage(t *testing.T) {
	type args struct {
		trc ylem_messaging.TaskRunContext
	}

	var mf *ylem_messaging.FilterTaskMessageFactory
	...
	correctTaskImplementation := &task.Filter{Uuid: "20469af1-3c50-499f-8b76-7d2ae0c66575", Expression: "id > 0"}
	...
	correctMessageEnvelope := &messaging.Envelope{
		Headers: map[string]string{"X-Message-Name": "tasks.filter"},
	...
	tests := []struct {
		name    string
		f       *ylem_messaging.FilterTaskMessageFactory
		args    args
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err != nil) != tt.wantErr {
				t.Errorf("FilterTaskMessageFactory.CreateMessage() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("FilterTaskMessageFactory.CreateMessage() = %v, want %v", got, tt.want)
			}
		})
	}
}
