...
import {HubspotAuthorizationInfo} from "../../../actions/infoTexts";
...
import HubspotAuthorizationForm from "../../../components/forms/hubspotAuthorizationForm.component";

const HUBSPOT_JUST_CONNECTED_QUERY_STRING = "?justConnected";

function withParams(Component) {
  return props => <Component {...props} params={useParams()} location={useLocation()} history={useNavigate()} />;
}

class HubspotAuthorizations extends React.Component {
    constructor(props) {
        super(props);
    ...
    componentDidMount = async() => {
        log.debug("HubspotAuthorizations component did mount")

        document.title = 'Hubspot Cloud Authorizations'
    ...
    handleCreateAuthorization = async(uuid) => {
        ...
        if (authorizationLink === null) {
            authorizationLink = IntegrationService.createHubspotAuthorization(uuid);
    ...
    handleGetAuthorizations = async(uuid) => {
        ...
        if (
            ...
            authorizations = IntegrationService.getHubspotAuthorizations(uuid);
    ...
    handleGetAuthorization = async(uuid) => {
        log.debug("handle get authorization");
            let authorization = IntegrationService.getHubspotAuthorization(uuid);
    ...
    closeForm = async() => {
        ...
        if (item !== null) {
            this.props.history('/hubspot-authorizations/');
        }
    }

    openForm = (item) => {
        log.debug("open form")
        this.props.history('/hubspot-authorizations/' + item.uuid);
    }
            ...
            <Fade>
                            ...
                            <Col sm="3" className="text-right">
                                <ButtonGroup className="mr-4">
                                    ...
                                    >
                                        ...
                                        <span>Add Hubspot authorization</span>
                                    </Button>
                                </ButtonGroup>
                            </Col>
                        </Row>
                ...
                {this.state.authorizations !== null ?
                    this.state.authorizations.length > 0 ?
                        ...
                        : <div className="text-center">You didn't create any Hubspot authorization yet</div>
                ...
                <InfoModal
                    ...
                    title={HubspotAuthorizationInfo.title}
                    content={HubspotAuthorizationInfo.content}
                />

                <FullScreenModal
                    show={isFormOpen}
                    onHide={this.closeForm}
                    title="Edit Hubspot Authorization"
                    content={
                        <>
                            <HubspotAuthorizationForm
                                item={activeItem}
                                successHandler={this.handleCloseFormAfterSuccess}
                                justConnected={search === HUBSPOT_JUST_CONNECTED_QUERY_STRING}
                            />
                        </>
                    }
                    item={activeItem}
...
export default connect(mapStateToProps)(withParams(HubspotAuthorizations));
